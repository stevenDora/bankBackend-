...
import cn.stylefeng.guns.modular.system.utils.IPAddressUtils;
...
public class LogAop {
    ...
    public Object recordSysLog(ProceedingJoinPoint point) throws Throwable {
        ...
        if (StringUtils.isNotEmpty(attributes)){
            ...
            log.info("请求method类型:{}", request.getMethod());
            log.info("请求IP:{}", IPAddressUtils.getIpAddress(request));
            log.info(StringUtils.format("请求方法全名 :{0}.{1}",
                    point.getSignature().getDeclaringTypeName(),
                    point.getSignature().getName()));
            log.info("请求参数值params : {}", Arrays.toString(point.getArgs()));
        }
        //先执行业务
